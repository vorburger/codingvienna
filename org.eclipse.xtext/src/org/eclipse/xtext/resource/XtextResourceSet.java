 ...
 * 
 * It maintains a {@link #getURIResourceMap()} containing the resource's URI as
 * well as the normalized version of it as a key. It assumes that the
 * {@link URI} of a resource never changes while contained in the
 * {@link #getResources()} list.
 * 
 * @author Jan Köhnlein
...
public class XtextResourceSet extends ResourceSetImpl {
	...
	protected void registerURI(Resource resource) {
		...
		if (map != null) {
			...
			if (previous != null && previous != resource) {
				throw new IllegalStateException(
						"A different resource with the URI '" + uri
	 ...
	 * {@link ResourceSetImpl#getURIResourceMap()}.
	 * 
	 * @since 2.3
	 */
	@SuppressWarnings("serial")
	protected class ResourcesList extends
			ResourceSetImpl.ResourcesEList<Resource> {
		@Override
		protected NotificationChain inverseAdd(Resource resource,
		...
		@Override
		protected NotificationChain inverseRemove(Resource resource,
				NotificationChain notifications) {
			final NotificationChain inverseRemove = super.inverseRemove(
			...
			uriConverter = new ExtensibleURIConverterImpl() {
				...
				public URI normalize(URI uri) {
					if (ClasspathUriUtil.isClasspathUri(uri)) {
						URI result = XtextResourceSet.this
